...
SELECT e.nome AS 'Nome da Editora', 
	ROUND(AVG(l.preco), 2) AS 'Média de preço dos Livros'
from editora e
INNER JOIN livro l ON e.idEditora = l.idEditora
GROUP BY e.nome;
...
SELECT c.nome AS "Nome do Cliente", 
       COUNT(ip.idLivro) AS "Quantidade de Livros Comprados"
FROM Cliente c
INNER JOIN Pedido p ON c.idCliente = p.idCliente
INNER JOIN ItemPedido ip ON p.idPedido = ip.idPedido  -- Juntando a tabela de livros, se aplicável
GROUP BY c.nome
ORDER BY c.nome;
